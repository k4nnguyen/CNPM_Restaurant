...
public class DishDAO extends DAO {
    ...
    public ArrayList<Dish> getAllDishes() {
        ...
        String sql = "SELECT id, dishCode, category, name, description, price, status "
                + "FROM tblDish WHERE status = ? ORDER BY dishCode";
    ...
    public ArrayList<Dish> searchDishes(String keyword) {
        ...
        String sql = "SELECT id, dishCode, category, name, description, price, status "
                + "FROM tblDish WHERE status = ? AND (dishCode LIKE ? OR name LIKE ?) "
                + "ORDER BY dishCode";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            String like = "%" + keyword + "%";
            ps.setString(1, Dish.STATUS_ACTIVE);
    ...
    public boolean addDish(Dish dish) {
        ...
        if (!isValidDish(dish) || isDishCodeExists(dish.getDishCode())) {
            return false;
        }
        String sql = "INSERT INTO tblDish(dishCode, category, name, description, price, status) "
                + "VALUES(?,?,?,?,?,?)";
    ...
    public boolean updateDish(Dish dish) {
        ...
        if (dish == null || dish.getId() <= 0 || !isValidDish(dish)) {
            return false;
        }
        String sql = "UPDATE tblDish SET category = ?, name = ?, description = ?, price = ? "
                + "WHERE id = ? AND status = ?";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
